...
# Documentos permitidos para mostrarse al usuario final
VISIBLE_DOC_TYPES = {
...
# Estados desde los que sí queremos tomar documentos públicos
...
def _profile_id(profile: dict) -> int:
    # compatibilidad si alguna parte vieja aún usa id además de user_id
    return int(_get_attr(profile, "id", "user_id", default=0) or 0)


def _get_latest_case(user_id: int) -> Optional[dict]:
    try:
...
def _get_public_cases(user_id: int) -> list[dict]:
    try:
        res = cases_table.query(
            IndexName="user_id-created_at-index",
            KeyConditionExpression=Key("user_id").eq(int(user_id)),
    ...
    return [
        c for c in items
        if str(_get_attr(c, "status", default="") or "").upper() in PUBLIC_CASE_STATUSES
    ]
...
def _latest_public_case_with_docs(user_id: int) -> Optional[dict]:
    for case in _get_public_cases(user_id):
        docs = _get_docs_for_case(int(_get_attr(case, "id", default=0) or 0))
        if docs:
            return case
    return None
...
def _serialize_worker(profile: dict) -> dict:
    ...
    latest = _get_latest_case(user_id)
    ...
    visible_documents = _public_docs_for_case(public_case, user_id) if public_case else []
...
def list_workers(
    ...
    for profile in rows:
        ...
        payload.append(_serialize_worker(profile))
...
def public_worker_document(worker_id: int, doc_id: int):
    ...
    public_case = _latest_public_case_with_docs(int(worker_id))
    if not public_case:
        raise HTTPException(status_code=404, detail="Documento público no encontrado.")

    docs = _get_docs_for_case(int(_get_attr(public_case, "id", default=0) or 0))
    d = next((doc for doc in docs if int(_get_attr(doc, "id", default=0) or 0) == int(doc_id)), None)
    ...
    filename = (
        ...
        or f"{_get_attr(d, 'doc_type', default='documento')}_{_get_attr(d, 'id', default=doc_id)}"
    )
